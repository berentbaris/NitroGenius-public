...
public partial class ActivityData : ScriptableObject {
	...
	public void CalculateBudget()
	{
		foreach (Activity item in _ActivityItems)
		{
			...
			{
				sectorList.list[0].Display_Budget += item._Budget - (Turn.Value * 0.5f * 60);
				sectorList.list[0].Product_Volume_Total += item._Product_Volume;
			}
			//Agriculture Budget
			if (item._Belonging_Sector_ID == 1)
			{
				sectorList.list[1].Display_Budget += item._Budget + (Turn.Value * 0.5f * 10);
				sectorList.list[1].Product_Volume_Total += item._Product_Volume;
			}
			//Consumers Budget
	...
	sectorList.list[0].Budget += sectorList.list[0].Display_Budget;
	sectorList.list[1].Budget += sectorList.list[1].Display_Budget * 3;
