...
public class UIGameEndRoleButton : MonoBehaviour
{
    ...
    {
        scoreText.text = "Your score is " + Mathf.RoundToInt(selection.score);
        //budget text
        if (selection != sectorlist.list[3] && selection != sectorlist.list[2])
        {
            budgetText.gameObject.SetActive(true);

            if (selection.Product_Volume_Total == selection.ProductVolumeTotalRecord[0])
            {
                budgetText.text = "- Your production remained the same";
            }
            else if (selection.Product_Volume_Total > selection.ProductVolumeTotalRecord[0])
            {
                float percentage = ((selection.Product_Volume_Total - selection.ProductVolumeTotalRecord[0]) / selection.ProductVolumeTotalRecord[0]) * 100;
                budgetText.text = "- Your production increased by " + Mathf.RoundToInt(percentage) + "%";
            }
            else if (selection.Product_Volume_Total < selection.ProductVolumeTotalRecord[0])
            {
                float percentage = ((selection.ProductVolumeTotalRecord[0] - selection.Product_Volume_Total) / selection.ProductVolumeTotalRecord[0]) * 100;
                budgetText.text = "- Your production decreased by " + Mathf.RoundToInt(percentage) + "%";
            }
        }
        else
        {
